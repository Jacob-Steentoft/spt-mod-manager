...
impl SptAccess {
	...
	pub fn backup_to<P: AsRef<Path>>(&self, archive_path: P) -> Result<()> {
		let mut zip_writer = create_backup_zipper(archive_path)?;
...
fn create_backup_zipper<P: AsRef<Path>>(archive_path: P) -> Result<ZipWriter<BufWriter<File>>> {
...
mod tests {
	...
	fn test_parse_download() {
		...
		SptAccess::new("./").install_for_client(buf).unwrap();
	}

	#[test]
	fn integration_test_backup() {
		let path = PathBuf::from("./test_output");
		SptAccess::new("./").backup_to(path).unwrap();
	}
